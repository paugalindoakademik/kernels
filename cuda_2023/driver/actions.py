...
def compile_benchmark(bmk, version_name, platform=None):
    ...
    #Clean the benchmark
    clean_benchmark(bmk, version_name, platform)
    print
...
def memcheck_benchmark(bmk, version_name, input_name, check=True, extra_opts=[], platform=None):
    """Run the benchmark under cuda-memcheck."""
    try: impl = bmk.impls[version_name]
    except KeyError:
        print "Cannot find benchmark version"
        return ErrorType.CannotFindVersion
    ...
    # Run the benchmark
    return impl.memcheck(bmk, data, check, extra_opts=extra_opts, platform=platform)
