...
def help_options(progname, cmd, args):
    ...
    def run():
        if args:
            ...
            print "  memcheck  Run a benchmark under cuda-memcheck"
            print ""
...
def debug_options(progname, cmd, args):
    ...
    def run():
        ...
        globals.verbose = opts.verbose
        if not len(pos) in [3, 4]:
            print "Expecting three or four parameters after 'debug'"
            return None
        else:
            bmkname = pos[0]
            ver = pos[1]
...
def memcheck_options(progname, cmd, args):
    usage_string = progname + " memcheck BENCHMARK VERSION INPUT\nRun version VERSION of BENCHMARK with data set INPUT under cuda-memcheck"
    parser = OptionParser(usage=usage_string)
    parser.add_option('-C', "--no-check",
                      action="store_false", dest="check", default=True,
                      help="Skip the output check for this benchmark")
    ...
    def run():
        ...
        globals.verbose = opts.verbose
        if not len(pos) in [3, 4]:
            print "Expecting three or four parameters after 'memcheck'"
            return None
        else:
            bmkname = pos[0]
            ver = pos[1]
            ...
            ck=opts.check
            extra_opts = []
            return lambda: actions.with_benchmark_named(bmkname, lambda b: actions.memcheck_benchmark(b, ver, inp, check=ck, extra_opts=extra_opts, platform=platform))
